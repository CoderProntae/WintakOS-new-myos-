...
static void send_eth(mac_addr_t dst, uint16_t ethertype, const void* payload, uint32_t len)
{
    ...
    uint32_t total = sizeof(eth_header_t) + len;
...
static void handle_icmp(const ip_header_t* ip_hdr, const uint8_t* data, uint32_t len)
{
    ...
    if (icmp->type == ICMP_ECHO_REQUEST) {
        ...
        rip->ver_ihl   = 0x45;
        rip->tos       = 0;
        rip->total_len = htons((uint16_t)(sizeof(ip_header_t) + reply_len));
        rip->id        = htons(ip_id_counter++);
        rip->frag_off  = 0;
        rip->ttl       = 64;
        ...
        memcpy(ricmp, data, reply_len);
        ricmp->type     = ICMP_ECHO_REPLY;
        ricmp->checksum = 0;
        ricmp->checksum = ip_checksum(ricmp, reply_len);

        /* TODO: ARP tablosundan MAC bul, simdilik broadcast */
        send_eth(broadcast_mac, ETH_TYPE_IP, reply_buf, sizeof(ip_header_t) + reply_len);
    }
...
void net_send_ping(ip_addr_t target_ip)
{
    ...
    send_eth(broadcast_mac, ETH_TYPE_IP, buf, sizeof(buf));
}

net_status_t* net_get_status(void) { return &status; }

void net_process(void) { /* Polling gerekmiyor, interrupt-driven */ }
