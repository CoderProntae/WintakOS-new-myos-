...
static void build_line(char* buf, const char* label, uint32_t val, const char* unit)
{
    uint32_t p = 0;
...
static void sysmon_draw(window_t* win)
{
    ...
    uint32_t used_pct = total_pages > 0 ? (used_pages * 100) / total_pages : 0;
    widget_draw_progress(win, 12, 68, 260, 14, used_pct, RGB(60, 140, 220), RGB(40, 40, 60));

    build_line(buf, "  Kullan\x01lan: ", used_pages, " sayfa");
    widget_draw_label(win, 12, 86, buf, RGB(180, 180, 180));
